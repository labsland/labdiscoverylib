...
.. labdiscoverylib documentation master file, created by
...
Creating a remote laboratory may imply many layers, such as authentication, authorization, scheduling, etc., so Remote Laboratory Management Systems (RLMS) were created to make the common layers of remote laboatories. LabDiscoveryEngine is an Open Source RLMS, and it has multiple ways (`see the docs <https://labsland.com/labdiscoveryengine>`_) to create a remote laboratory (in different programming languages, etc.).

In the case of Python, with the popular `Flask <http://flask.pocoo.org>`_ microframework, labdiscoverylib is the wrapper used to create unmanaged labs. Unmanaged labs is a term used in LabDiscoveryEngine to refer laboratories where the authors develop the full stack (server, client, deployment), as opposed to managed labs.
