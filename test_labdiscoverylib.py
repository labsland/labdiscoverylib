...
import labdiscoverylib
from labdiscoverylib.tasks import _TaskRunner
import unittest
...
class WithoutWebLabTest(unittest.TestCase):
    def test_extension(self):
        app = Flask(__name__)
        with self.assertRaises(labdiscoverylib.WebLabNotInitializedError):
            with app.app_context():
                labdiscoverylib._current_weblab()
...
class BaseWebLabTest(unittest.TestCase):
    ...
    def create_weblab(self):
        labdiscoverylib._CleanerThread.created = False
        self.weblab = labdiscoverylib.WebLab()
        ...
        @self.app.route('/lab/')
        @labdiscoverylib.requires_login
        def lab():
            return self.lab()

        @self.app.route('/lab/active')
        @labdiscoverylib.requires_active
        def lab_active():
            return self.lab()
        ...
        @labdiscoverylib.requires_active
        def logout():
            labdiscoverylib.logout()
            return "logout"

        @self.app.route('/poll')
        @labdiscoverylib.requires_active
        def poll():
            labdiscoverylib.poll()
            labdiscoverylib.poll() # Twice so as to test g.poll_requested
            return "poll"
        ...
        with self.assertRaises(ValueError) as cm:
            ...
            self.assertEquals(43, len(labdiscoverylib.create_token()))
...
class SimpleUnauthenticatedTest(BaseWebLabTest):
    ...
    def test_anonymous(self):
        with self.app.test_client() as client:
            client.get('/lab/')
            self.assertTrue(labdiscoverylib.weblab_user.is_anonymous)
            self.assertFalse(labdiscoverylib.weblab_user.active)
            self.assertIsNone(labdiscoverylib.weblab_user.locale)
            self.assertIsNotNone(labdiscoverylib.weblab_user.data)
            with self.assertRaises(TypeError):
                self.assertEquals(0, len(labdiscoverylib.weblab_user.data))
...
class UnauthorizedTemplateSimpleTest(BaseWebLabTest):
    ...
    def test_unauthorized_template(self):
        ...
        old_render_template = labdiscoverylib.render_template

        with self.app.test_client() as client:
            labdiscoverylib.render_template = new_render_template
            try:
                lab_result = self.get_text(client.get('/lab/'))
            finally:
                labdiscoverylib.render_template = old_render_template
...
class UserTest(BaseSessionWebLabTest):
    ...
    def lab(self):
        ...
        if labdiscoverylib.weblab_user.active:
            labdiscoverylib.weblab_user.data['foo'] = 'bar'
    ...
    def task(self):
        self.counter += 1
        if not labdiscoverylib.current_task_stopping:
            return "ERROR"
        labdiscoverylib.current_task.data = {'inside': 'previous'}
        labdiscoverylib.current_task.update_data({'inside': 'yes'})
        labdiscoverylib.current_task.store()
        labdiscoverylib.weblab_user.data.retrieve()
        return [ self.counter, labdiscoverylib.weblab_user.data['foo'] ]
    ...
    def test_simple(self):
        ...
        self.assertEquals(labdiscoverylib.weblab_user.session_id, session_id1)
        self.assertEquals(labdiscoverylib.weblab_user.locale, 'es_ES')
        self.assertEquals(labdiscoverylib.weblab_user.full_name, 'Jim Smith')
        self.assertEquals(labdiscoverylib.weblab_user.experiment_name, 'mylab')
        ...
        self.assertFalse(labdiscoverylib.current_task_stopping)
        ...
        self.assertFalse(labdiscoverylib.current_task)
        ...
        self.client.get('/lab/')
        self.assertFalse(labdiscoverylib.weblab_user.active)
        self.assertFalse(labdiscoverylib.weblab_user.is_anonymous)
        self.assertEquals(labdiscoverylib.weblab_user.time_left, 0)
        self.assertEquals(labdiscoverylib.weblab_user.session_id, session_id1)
        self.assertIn(session_id1, str(labdiscoverylib.weblab_user))
        with self.assertRaises(NotImplementedError):
            labdiscoverylib.weblab_user.data = {}

        with self.assertRaises(NotImplementedError):
            labdiscoverylib.weblab_user.update_data()
        self.assertEquals(labdiscoverylib.weblab_user.locale, 'es_ES')
        self.assertEquals(labdiscoverylib.weblab_user.full_name, 'Jim Smith')
        self.assertEquals(labdiscoverylib.weblab_user.experiment_name, 'mylab')
        self.assertEquals(labdiscoverylib.weblab_user.category_name, 'Lab experiments')
...
class TaskJoinTimeoutTest(BaseSessionWebLabTest):
    ...
    def test_task_join_timeout(self):
        ...
        with self.assertRaises(labdiscoverylib.TimeoutError):
...
class TaskJoinSameThreadTest(BaseSessionWebLabTest):
    ...
    def task(self):
        labdiscoverylib.current_task.join(timeout=0.5)
...
class LoadUserTest(BaseSessionWebLabTest):
    def test_load_no_loader(self):
        ...
        user = labdiscoverylib.weblab_user.user
        self.assertIsNone(user)

    def test_load_simple(self):
        @self.weblab.user_loader
        def user_loader(username_unique):
            return MyLabUser(username_unique, labdiscoverylib.weblab_user.username)
        ...
        user2 = labdiscoverylib.weblab_user.user
        ...
        user3 = labdiscoverylib.weblab_user.user
    ...
    def test_user_loader_fail(self):
        ...
        with self.assertRaises(Exception) as cm:
            labdiscoverylib.weblab_user.user
...
class GlobalUniqueTaskTest(BaseSessionWebLabTest):

    def get_config(self):
        config = super(GlobalUniqueTaskTest, self).get_config()
        config['WEBLAB_AUTOCLEAN_THREAD'] = True
        config['WEBLAB_TASK_THREADS_PROCESS'] = 3
        return config
    ...
    def task(self):
        ...
        while not labdiscoverylib.current_task_stopping:
            time.sleep(0.05)
...
class UserUniqueTaskTest(BaseSessionWebLabTest):
    ...
    def get_config(self):
        config = super(UserUniqueTaskTest, self).get_config()
        config['WEBLAB_AUTOCLEAN_THREAD'] = True
        config['WEBLAB_TASK_THREADS_PROCESS'] = 3
        return config
    ...
    def lab(self):
        ...
        task2.join() # Should be fast
        if not task2.failed:
            raise Exception("unique=user should have avoided the second task")
        if task2.error['class'] != 'AlreadyRunningError':
            raise Exception("unique=user expected an AlreadyRunningError; got {}".format(task2.error))
        return str(task_object.task_id)
...
class StartErrorTest(BaseSessionWebLabTest):
    ...
    def test_task_fail(self):
        import labdiscoverylib.views as labdiscoverylib_views
        old_dispose_user = labdiscoverylib_views.dispose_user
        def new_dispose_user(*args, **kwargs):
            raise Exception("weird error")

        labdiscoverylib_views.dispose_user = new_dispose_user
        try:
            with self.assertRaises(TestNewUserError):
                with StdWrap():
                    self.new_user()
        finally:
            labdiscoverylib_views.dispose_user = old_dispose_user
...
class BaseCLITest(BaseSessionWebLabTest):
    ...
    def setUp(self):
        ...
        labdiscoverylib.requests = FakeRequests

    def tearDown(self):
        super(BaseCLITest, self).tearDown()
        labdiscoverylib.requests = requests
...
class CLITest(BaseCLITest):
    ...
    def test_cli_flow(self):
        ...
        labdiscoverylib.webbrowser = webbrowser
    ...
    def test_loop_cli(self):
        ...
        labdiscoverylib._TESTING_LOOP = True
...
class WebLabSetupErrorsTest(unittest.TestCase):
    def test_empty_app(self):
        with self.assertRaises(ValueError) as cm:
            labdiscoverylib.WebLab().init_app(None)
    ...
    def test_app_trailing_slashes(self):
        ...
        with StdWrap():
            weblab = labdiscoverylib.WebLab(app)
        weblab._cleanup()
    ...
    def test_app_twice_different_apps(self):
        ...
        app2 = Flask(__name__)
        weblab = labdiscoverylib.WebLab(app1)
        with self.assertRaises(ValueError) as cm:
    ...
    def test_app_two_weblabs_same_app(self):
        ...
        weblab1 = labdiscoverylib.WebLab(app)
        with self.assertRaises(ValueError) as cm:
            weblab2 = labdiscoverylib.WebLab(app)
    ...
    def _create_weblab(self):
        ...
        self.weblab = labdiscoverylib.WebLab(self.app)
    ...
    def test_socket_requires_login_without_socketio(self):
        past_value = labdiscoverylib._FLASK_SOCKETIO
        labdiscoverylib._FLASK_SOCKETIO = False
        try:
            with StdWrap():
                @labdiscoverylib.socket_requires_login
                def f():
            ...
            labdiscoverylib._FLASK_SOCKETIO = past_value
